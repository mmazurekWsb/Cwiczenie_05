    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            int[,] array =
            {
                ...
                {10,11,12,13,14 },
                {1,2,3,4,5 }];
            znajdzNajbardziePodobneWiersze(array, out int row1, out int row2);
            Console.WriteLine($"Najbardziej podobne wiersze:{row1} i {row2}");

            }
        static void znajdzNajbardziePodobneWiersze(int[,] array, out int row1, out int row2)
        {
            row1 = 0;
            row2 = 0;
            ...
            for(int i=0; i <iloscWierszy; i++)
            {
                ...
                {
                    int roznica = wyliczRoznice(array, i, j);
        ...
        static void wyliczRoznice(int[,] array, int i , int j)
        {
            int iloscKolumn = array.GetLength(1);
            int roznica = 0;
